...
use heterion_router_core::{ChatCompletionRequest, GatewayError, StreamChunk};
...
mod tests {
    ...
    fn open_state() -> (Router, Arc<heterion_router_db::Db>) {
    ...
    fn keyed_state() -> (Router, Arc<heterion_router_db::Db>) {
        let db = Arc::new(heterion_router_db::Db::open_in_memory().unwrap());
        db.migrate().unwrap();
        {
            let guard = db.connection();
